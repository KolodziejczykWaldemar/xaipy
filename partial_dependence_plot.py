...
def plot_pdp(features, model, feature_number, feature_name):
    # TODO add types of variables, continous and discrete
    # TODO add docstring
    ...
    sampled_values = np.linspace(start=min_value, stop=max_value, num=sample_size)
    ...
    for i, row in enumerate(other_features):
        copied_instances = np.repeat(row.reshape(1, -1), sample_size, axis=0)
    ...
    return mean_outputs['output'].values


def plot_ice(features, model, feature_number, feature_name):
    # TODO add types of variables, continous and discrete
    # TODO add docstring
    sample_indexes = np.random.randint(features.shape[0], size=1000)
    features = features[sample_indexes, :]
    ...
    for i, row in enumerate(other_features):
        ...
        row_indicator += ((np.ones(sample_size) * i).ravel().tolist())
